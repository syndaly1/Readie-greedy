...
function renderGrid(items) {
  if (!items.length) {
    grid.innerHTML = `<div class="card">No books found. Humans always want “filters”, then complain nothing matches.</div>`;
    return;
  }
  ...
  grid.innerHTML = items.map((b) => {
    ...
    const rating = (b.rating ?? null) !== null ? `★ ${esc(b.rating)}` : "★ —";
    const tags = Array.isArray(b.tags) ? b.tags.slice(0, 4).map(t => `<span class="pill">${esc(t)}</span>`).join("") : "";
...
async function showDetails(id) {
  ...
  if (!res.ok) return openModal(`<h2>Error</h2><p>${esc(b.error || "Failed")}</p>`);

  const tags = Array.isArray(b.tags) ? b.tags.map(t => `<span class="pill">${esc(t)}</span>`).join("") : "";
  openModal(`
    <h2>${esc(b.title)}</h2>
    <p class="muted-text">by ${esc(b.author)} ${b.year ? `· ${esc(b.year)}` : ""}</p>
    <div class="book-tags">${tags}</div>
    <p style="margin-top:14px; line-height:1.7">${esc(b.description || "No description.")}</p>
    ...
    <form id="editForm" class="form">
      ...
      <div class="two-col">
        <div>
          <label>Title *</label>
          <input class="input" name="title" required value="${esc(b.title)}" />
        </div>
        <div>
          <label>Author *</label>
          <input class="input" name="author" required value="${esc(b.author)}" />
        </div>
      </div>
      ...
      <div class="two-col">
        <div>
          <label>Year</label>
          <input class="input" name="year" type="number" value="${esc(b.year ?? "")}" />
        </div>
        <div>
          <label>Rating</label>
          <input class="input" name="rating" type="number" step="0.1" min="0" max="5" value="${esc(b.rating ?? "")}" />
        </div>
      </div>

      <label>Tags (comma-separated)</label>
      <input class="input" name="tags" value="${esc((b.tags || []).join(", "))}" />

      <label>Description</label>
      <textarea class="input" name="description" rows="4">${esc(b.description || "")}</textarea>

      <button class="btn" type="submit">Save (PUT)</button>
  ...
  editForm.addEventListener("submit", async (e) => {
    ...
    if (!r.ok) {
      editMsg.textContent = out.error || "Failed";
      editMsg.className = "form-msg bad";
      return;
    }
...
async function deleteBook(id) {
  ...
  if (!headers.Authorization) return requireLogin();
...
createForm.addEventListener("submit", async (e) => {
  ...
  if (!res.ok) {
    formMsg.textContent = out.error || "Failed";
    formMsg.className = "form-msg bad";
    return;
  }

  formMsg.textContent = `Created: ${out.id}`;
